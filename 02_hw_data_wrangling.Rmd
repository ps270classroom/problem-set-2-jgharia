...
```{r}
#num_candidates <- (fb_ads$cand_name)
...
Democratic Incumbent sponsored the most promoting ads on average. 
...
plot_q3 <- ggplot(us_house_ads, aes(x = impressions_millions)) +
  geom_histogram() + 
  labs(title = "Histogram of Facebook Ad Impressions (Millions) fror US House Races", 
       x = "Impression (Millions)", 
       y = "Count") 
...
```{r}
fb_top_plot <- ggplot(top_15_candidates, 
...
fb_top_plot <- ggplot(top_cand, aes(x = impressions_millions, 
  ...
  scale_fill_manual(values = c("R" = "red", "D" = "steelblue4"))
